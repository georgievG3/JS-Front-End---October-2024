...
function solve() {
    ...
    function handleSubmitEvent(e) {
        ...
        const currrentInputEl = e.target.querySelector('input[type="number"]')
        const currentValue = Number(currrentInputEl.value)

        if (currentValue < 1 ) return

        const key = currrentInputEl.getAttribute('id').split('-input')[0]
